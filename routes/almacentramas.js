...
router.get('/almacentramas/:id', function (req, res) {
  almacentramas.findOne({ 'id_formato': req.params.id }, function (err, data) {
    res.json(data);
  })

})

router.post('/almacentramas/eliminarGrabar', function (req, res) {
  /*almacentramas.findOne({ 'id_formato': req.params.id }, function (err, data) {
    res.json(data);
  })*/
  if(req.body.idMongo){
    almacentramas.findByIdAndRemove(req.body.idMongo, function(err){
         if(err){res.json(err)}
         var model = new almacentramas(req.body);
         model.save(function(err, data){
           if(err){return next(err)}
           res.json('Se ha grabado exitosamente!!');
           meanCaseBase.auditSave(req,'Trama Modificada','Test Parsing Individual',data);
         })
   })
  }else{
      var model = new almacentramas(req.body);
